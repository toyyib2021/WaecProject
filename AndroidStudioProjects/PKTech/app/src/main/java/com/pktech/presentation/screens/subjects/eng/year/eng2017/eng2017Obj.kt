...
fun Eng2017Obj(
    ...
    val english2017VM: English2017VM = hiltViewModel()
    ...
    val english2017 by english2017VM.getEnglish2017.observeAsState(listOf<English>())
    val english2017R = english2017VM.getEnglish2017.value
    english2017::class.java

    var currentIndex by remember { mutableStateOf(0) }

    val questionIndex = english2017R?.english?.get(currentIndex)?.objective?.id
    val currentQuestion = english2017R?.english?.get(currentIndex)?.objective?.question
    val mainOptionA = english2017R?.english?.get(currentIndex)?.objective?.optionA
    val mainOptionB = english2017R?.english?.get(currentIndex)?.objective?.optionB
    val mainOptionC = english2017R?.english?.get(currentIndex)?.objective?.optionC
    val mainOptionD = english2017R?.english?.get(currentIndex)?.objective?.optionD
    val answer = english2017R?.english?.get(currentIndex)?.objective?.explanation
    val currentInstructions = english2017R?.english?.get(currentIndex)?.objective?.instructions
    val correctOption = english2017R?.english?.get(currentIndex)?.objective?.correctOption
    val underline = english2017R?.english?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = english2017R?.english?.get(currentIndex)?.objective?.questionEnd

    subjectVM.addSaveQuestionData.value.questionTitle = SaveQuestionConstants.ENGLISH2017
    ...
    BottomSheetScaffold(
        ...
        Scaffold(
            topBar = {
                ...
                val splitList = SaveQuestionConstants.ENGLISH2017.split(" ")
                ...
                LaunchedEffect(key1 = resultKey){
                    if(resultKey == 2){
                        ...
                        var index = 0
                        var result = 0
                        if (english2017R != null) {
                            val correctOption2 = english2017R.english.get(index).objective.correctOption
                            val selectedOption = getSelectedOptionR?.get(index)?.selectedOption
                            while (index < english2017R.english.size){
                                if (english2017R.english.get(index).objective.correctOption ==
                        ...
                        finailResult = result
                        if (english2017R != null) {
                            subjectVM.addTestTimeline.value.testResult = "$result / ${english2017R.english.size}"
                        }
                ...
                DisplayAlertDialog(
                    ...
                    onYesClickedTest = {
                        scope.launch {
                            questionTitleKey.saveKey(SaveQuestionConstants.ENGLISH2017)
                        }
                        resultKey = 2
                        onYesClickTest() },
                ...
                StudyTopBar(
                    ...
                    questionTitle = SaveQuestionConstants.ENGLISH2017,
            ...
            if (currentInstructions != null && questionIndex != null) {
                ...
                StudyObjUIItems(
                    ...
                    questionSize = english2017R.english.size.toString(),
                    studyOrTestState = studyOrTestKeyValue.value,
                    onSaveIconClick = {
