...
fun Eng2018Obj(
    ...
    val english2018VM: English2018VM = hiltViewModel()
    ...
    val english2018 by english2018VM.getEnglish2018.observeAsState(listOf<English>())
    val english2018R = english2018VM.getEnglish2018.value
    english2018::class.java

    var currentIndex by remember { mutableStateOf(0) }

    val questionIndex = english2018R?.english?.get(currentIndex)?.objective?.id
    val currentQuestion = english2018R?.english?.get(currentIndex)?.objective?.question
    val mainOptionA = english2018R?.english?.get(currentIndex)?.objective?.optionA
    val mainOptionB = english2018R?.english?.get(currentIndex)?.objective?.optionB
    val mainOptionC = english2018R?.english?.get(currentIndex)?.objective?.optionC
    val mainOptionD = english2018R?.english?.get(currentIndex)?.objective?.optionD
    val answer = english2018R?.english?.get(currentIndex)?.objective?.explanation
    val currentInstructions = english2018R?.english?.get(currentIndex)?.objective?.instructions
    val correctOption = english2018R?.english?.get(currentIndex)?.objective?.correctOption
    val underline = english2018R?.english?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = english2018R?.english?.get(currentIndex)?.objective?.questionEnd

    subjectVM.addSaveQuestionData.value.questionTitle = SaveQuestionConstants.ENGLISH2018
    ...
    BottomSheetScaffold(
        ...
        Scaffold(
            topBar = {
                ...
                val splitList = SaveQuestionConstants.ENGLISH2018.split(" ")
                ...
                LaunchedEffect(key1 = resultKey){
                    if(resultKey == 2){
                        ...
                        var index = 0
                        var result = 0
                        if (english2018R != null) {
                            val correctOption2 = english2018R.english.get(index).objective.correctOption
                            val selectedOption = getSelectedOptionR?.get(index)?.selectedOption
                            while (index < english2018R.english.size){
                                if (english2018R.english.get(index).objective.correctOption ==
                        ...
                        finailResult = result
                        if (english2018R != null) {
                            subjectVM.addTestTimeline.value.testResult = "$result / ${english2018R.english.size}"
                        }
                ...
                DisplayAlertDialog(
                    ...
                    onYesClickedTest = {
                        scope.launch {
                            questionTitleKey.saveKey(SaveQuestionConstants.ENGLISH2018)
                        }
                        resultKey = 2
                        onYesClickTest() },
                ...
                StudyTopBar(
                    ...
                    questionTitle = SaveQuestionConstants.ENGLISH2018,
            ...
            if (currentInstructions != null && questionIndex != null) {
                ...
                StudyObjUIItems(
                    ...
                    questionSize = english2018R.english.size.toString(),
                    studyOrTestState = studyOrTestKeyValue.value,
                    onSaveIconClick = {
