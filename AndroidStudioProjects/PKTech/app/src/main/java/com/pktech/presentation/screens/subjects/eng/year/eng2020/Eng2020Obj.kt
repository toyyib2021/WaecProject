...
fun Eng2020Obj(
    ...
    val english2020VM: English2020VM = hiltViewModel()
    ...
    val english2020 by english2020VM.getEnglish2020.observeAsState(listOf<English>())
    val english2020R = english2020VM.getEnglish2020.value
    english2020::class.java

    var currentIndex by remember { mutableStateOf(0) }

    val questionIndex = english2020R?.english?.get(currentIndex)?.objective?.id
    val currentQuestion = english2020R?.english?.get(currentIndex)?.objective?.question
    val mainOptionA = english2020R?.english?.get(currentIndex)?.objective?.optionA
    val mainOptionB = english2020R?.english?.get(currentIndex)?.objective?.optionB
    val mainOptionC = english2020R?.english?.get(currentIndex)?.objective?.optionC
    val mainOptionD = english2020R?.english?.get(currentIndex)?.objective?.optionD
    val answer = english2020R?.english?.get(currentIndex)?.objective?.explanation
    val currentInstructions = english2020R?.english?.get(currentIndex)?.objective?.instructions
    val correctOption = english2020R?.english?.get(currentIndex)?.objective?.correctOption
    val underline = english2020R?.english?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = english2020R?.english?.get(currentIndex)?.objective?.questionEnd



    subjectVM.addSaveQuestionData.value.questionTitle = SaveQuestionConstants.ENGLISH2020
    ...
    BottomSheetScaffold(
        ...
        Scaffold(
            topBar = {
                ...
                val splitList = SaveQuestionConstants.ENGLISH2020.split(" ")
                ...
                LaunchedEffect(key1 = resultKey){
                    if(resultKey == 2){
                        ...
                        var index = 0
                        var result = 0
                        if (english2020R != null) {
                            val correctOption2 = english2020R.english.get(index).objective.correctOption
                            val selectedOption = getSelectedOptionR?.get(index)?.selectedOption
                            while (index < english2020R.english.size){
                                if (english2020R.english.get(index).objective.correctOption ==
                        ...
                        finailResult = result
                        if (english2020R != null) {
                            subjectVM.addTestTimeline.value.testResult = "$result / ${english2020R.english.size}"
                        }
                ...
                DisplayAlertDialog(
                    ...
                    onYesClickedTest = {
                        scope.launch {
                            questionTitleKey.saveKey(SaveQuestionConstants.ENGLISH2020)
                        }
                        resultKey = 2
                        onYesClickTest() },
                ...
                StudyTopBar(
                    ...
                    questionTitle = SaveQuestionConstants.ENGLISH2020,
            ...
            if (currentInstructions != null && questionIndex != null) {
                ...
                StudyObjUIItems(
                    ...
                    questionSize = english2020R.english.size.toString(),
                    studyOrTestState = studyOrTestKeyValue.value,
                    onSaveIconClick = {
