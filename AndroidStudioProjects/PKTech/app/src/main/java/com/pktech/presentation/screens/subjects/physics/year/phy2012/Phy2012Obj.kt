package com.pktech.presentation.screens.subjects.physics.year.phy2012
...
import com.pktech.navigation.screens.LiteratureObjYear
import com.pktech.navigation.screens.PhysicsObjYear
...
import com.pktech.presentation.screens.subjects.litineng.year.liteng2020.Literature2020VM
import com.pktech.presentation.screens.subjects.physics.year.phy2012.items.PhysicsQuestion
...
import com.pktech.utill.SaveQuestionConstants.PHYSICS2012
...
fun Phy2012Obj(
    ...
    val physicsVM: Physics2012VM = hiltViewModel()
    ...
    val questionTitle = PHYSICS2012
    val questionRoute = PhysicsObjYear.Obj2012.route
    ...
    val physicsQuestion by physicsVM.getPhysics2012.observeAsState(listOf<Physics>())
    val physicsQuestionR = physicsVM.getPhysics2012.value
    physicsQuestion::class.java
    ...
    val questionSize = physicsQuestionR?.physics?.size
    ...
    val questionIndex = physicsQuestionR?.physics?.get(currentIndex)?.objective?.id
    val currentQuestion = physicsQuestionR?.physics?.get(currentIndex)?.objective?.question
    val mainOptionA = physicsQuestionR?.physics?.get(currentIndex)?.objective?.optionA
    val mainOptionB = physicsQuestionR?.physics?.get(currentIndex)?.objective?.optionB
    val mainOptionC = physicsQuestionR?.physics?.get(currentIndex)?.objective?.optionC
    val mainOptionD = physicsQuestionR?.physics?.get(currentIndex)?.objective?.optionD
    val answer = physicsQuestionR?.physics?.get(currentIndex)?.objective?.explanation
    val currentInstructions = physicsQuestionR?.physics?.get(currentIndex)?.objective?.instructions
    val correctOption = physicsQuestionR?.physics?.get(currentIndex)?.objective?.correctOption
    val underline = physicsQuestionR?.physics?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = physicsQuestionR?.physics?.get(currentIndex)?.objective?.questionEnd
