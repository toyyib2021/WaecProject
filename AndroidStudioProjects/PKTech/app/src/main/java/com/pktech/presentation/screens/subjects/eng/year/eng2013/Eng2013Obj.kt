package com.pktech.presentation.screens.subjects.eng.year.eng2013
...
fun Eng2013Obj(
    ...
    val english2013VM: English2013VM = hiltViewModel()
    ...
    val english2013 by english2013VM.getEnglish2013.observeAsState(listOf<English>())
    val english2013R = english2013VM.getEnglish2013.value
    english2013::class.java

    var currentIndex by remember { mutableStateOf(0) }

    val questionIndex = english2013R?.english?.get(currentIndex)?.objective?.id
    val currentQuestion = english2013R?.english?.get(currentIndex)?.objective?.question
    val mainOptionA = english2013R?.english?.get(currentIndex)?.objective?.optionA
    val mainOptionB = english2013R?.english?.get(currentIndex)?.objective?.optionB
    val mainOptionC = english2013R?.english?.get(currentIndex)?.objective?.optionC
    val mainOptionD = english2013R?.english?.get(currentIndex)?.objective?.optionD
    val answer = english2013R?.english?.get(currentIndex)?.objective?.explanation
    val currentInstructions = english2013R?.english?.get(currentIndex)?.objective?.instructions
    val correctOption = english2013R?.english?.get(currentIndex)?.objective?.correctOption
    val underline = english2013R?.english?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = english2013R?.english?.get(currentIndex)?.objective?.questionEnd


    subjectVM.addSaveQuestionData.value.questionTitle = SaveQuestionConstants.ENGLISH2013
    ...
    BottomSheetScaffold(
        ...
        Scaffold(
            topBar = {
                ...
                val splitList = SaveQuestionConstants.ENGLISH2013.split(" ")
                ...
                LaunchedEffect(key1 = resultKey){
                    if(resultKey == 2){
                        ...
                        var index = 0
                        var result = 0
                        if (english2013R != null) {
                            val correctOption2 = english2013R.english.get(index).objective.correctOption
                            val selectedOption = getSelectedOptionR?.get(index)?.selectedOption
                            while (index < english2013R.english.size){
                                if (english2013R.english.get(index).objective.correctOption ==
                        ...
                        finailResult = result
                        if (english2013R != null) {
                            subjectVM.addTestTimeline.value.testResult = "$result / ${english2013R.english.size}"
                        }
                ...
                DisplayAlertDialog(
                    ...
                    onYesClickedTest = {
                        scope.launch {
                            questionTitleKey.saveKey(ENGLISH2013)
                        }
                        resultKey = 2
                        onYesClickTest() },
                ...
                StudyTopBar(
                    ...
                    questionTitle = SaveQuestionConstants.ENGLISH2013,
            ...
            if (currentInstructions != null && questionIndex != null) {
                ...
                StudyObjUIItems(
                    ...
                    questionSize = english2013R.english.size.toString(),
                    studyOrTestState = studyOrTestKeyValue.value,
                    onSaveIconClick = {
