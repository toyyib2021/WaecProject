package com.pktech.presentation.screens.subjects.eng.year.eng2016
...
fun Eng2016Obj(
    ...
    val english2016VM: English2016VM = hiltViewModel()
    ...
    val english2016 by english2016VM.getEnglish2016.observeAsState(listOf<English>())
    val english2016R = english2016VM.getEnglish2016.value
    english2016::class.java

    var currentIndex by remember { mutableStateOf(0) }

    val questionIndex = english2016R?.english?.get(currentIndex)?.objective?.id
    val currentQuestion = english2016R?.english?.get(currentIndex)?.objective?.question
    val mainOptionA = english2016R?.english?.get(currentIndex)?.objective?.optionA
    val mainOptionB = english2016R?.english?.get(currentIndex)?.objective?.optionB
    val mainOptionC = english2016R?.english?.get(currentIndex)?.objective?.optionC
    val mainOptionD = english2016R?.english?.get(currentIndex)?.objective?.optionD
    val answer = english2016R?.english?.get(currentIndex)?.objective?.explanation
    val currentInstructions = english2016R?.english?.get(currentIndex)?.objective?.instructions
    val correctOption = english2016R?.english?.get(currentIndex)?.objective?.correctOption
    val underline = english2016R?.english?.get(currentIndex)?.objective?.questionUnderline
    val endQuestion = english2016R?.english?.get(currentIndex)?.objective?.questionEnd


    subjectVM.addSaveQuestionData.value.questionTitle = SaveQuestionConstants.ENGLISH2016
    ...
    BottomSheetScaffold(
        ...
        Scaffold(
            topBar = {
                ...
                val splitList = SaveQuestionConstants.ENGLISH2016.split(" ")
                ...
                LaunchedEffect(key1 = resultKey){
                    if(resultKey == 2){
                        ...
                        var index = 0
                        var result = 0
                        if (english2016R != null) {
                            val correctOption2 = english2016R.english.get(index).objective.correctOption
                            val selectedOption = getSelectedOptionR?.get(index)?.selectedOption
                            while (index < english2016R.english.size){
                                if (english2016R.english.get(index).objective.correctOption ==
                        ...
                        finailResult = result
                        if (english2016R != null) {
                            subjectVM.addTestTimeline.value.testResult = "$result / ${english2016R.english.size}"
                        }
                ...
                DisplayAlertDialog(
                    ...
                    onYesClickedTest = {
                        scope.launch {
                            questionTitleKey.saveKey(SaveQuestionConstants.ENGLISH2016)
                        }
                        resultKey = 2
                        onYesClickTest() },
                ...
                StudyTopBar(
                    ...
                    questionTitle = SaveQuestionConstants.ENGLISH2016,
            ...
            if (currentInstructions != null && questionIndex != null) {
                ...
                StudyObjUIItems(
                    ...
                    questionSize = english2016R.english.size.toString(),
                    studyOrTestState = studyOrTestKeyValue.value,
                    onSaveIconClick = {
